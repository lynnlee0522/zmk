...
LV_FONT_DECLARE(font_24_big_john);
...
int zmk_widget_logo_init(struct zmk_widget_logo *widget, lv_obj_t *parent) {
    ...
    lv_obj_set_height(main_container, 50);
    lv_obj_set_width(main_container, lv_obj_get_width(parent));
    lv_obj_set_style_border_width(main_container, 0, 0);
    lv_obj_set_style_pad_all(main_container, 0, 0);
    // 设置主容器背景为深蓝色
    lv_obj_set_style_bg_color(main_container, lv_color_hex(0x000080), LV_PART_MAIN);
    ...
    lv_obj_set_style_text_font(label, &font_24_big_john, LV_PART_MAIN);
    // 设置标签颜色为橙色
